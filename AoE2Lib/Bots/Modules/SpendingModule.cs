using AoE2Lib.Utils;
using PeNet.PatternMatching;
using System;
using System.Collections.Generic;
using System.Text;

namespace AoE2Lib.Bots.Modules
{
    public class SpendingModule : Module
    {
        public class SpendingCommand : Command
        {
            public string Name { get; set; } = "null";
            public int Priority { get; set; } = 0;
            public int WoodCost { get; set; } = 0;
            public int FoodCost { get; set; } = 0;
            public int GoldCost { get; set; } = 0;
            public int StoneCost { get; set; } = 0;
            public int Cost => WoodCost + FoodCost + GoldCost + StoneCost;
        }

        private readonly List<SpendingCommand> Commands = new List<SpendingCommand>();

        public void Add(SpendingCommand command)
        {
            Commands.Add(command);
        }

        protected override IEnumerable<Command> RequestUpdate()
        {
            var info = Bot.GetModule<InfoModule>();
            var wood = info.WoodAmount;
            var food = info.FoodAmount;
            var gold = info.GoldAmount;
            var stone = info.StoneAmount;
            var priority = 0;

            var wood_shortage = false;
            var food_shortage = false;
            var gold_shortage = false;
            var stone_shortage = false;

            foreach (var command in Commands)
            {
                Bot.Log.Info($"SpendingModule: Command {command.Name} cost {command.Cost} priority {command.Priority}");
                if (command.Cost <= 0)
                {
                    yield return command;
                }
                else
                {
                    wood -= command.WoodCost;
                    food -= command.FoodCost;
                    gold -= command.GoldCost;
                    stone -= command.StoneCost;

                    var spend = true;
                    if (wood < 0 || food < 0 || gold < 0 || stone < 0)
                    {
                        spend = false;

                        if (wood < 0)
                        {
                            wood_shortage = true;
                            Bot.Log.Info("SpendingModule: Wood shortage");
                        }
                        if (food < 0)
                        {
                            food_shortage = true;
                            Bot.Log.Info("SpendingModule: Food shortage");
                        }
                        if (gold < 0)
                        {
                            gold_shortage = true;
                            Bot.Log.Info("SpendingModule: Gold shortage");
                        }
                        if (stone < 0)
                        {
                            stone_shortage = true;
                            Bot.Log.Info("SpendingModule: Stone shortage");
                        }

                        priority = Math.Max(priority, command.Priority + 1);
                        Bot.Log.Info($"SpendingModule: New min priority {priority}");
                    }

                    if (spend && command.Priority < priority)
                    {
                        Bot.Log.Info($"SpendingModule: Command {command.Name} priority {command.Priority} below min priority {priority}");
                        if (command.WoodCost > 0 && wood_shortage)
                        {
                            spend = false;
                            Bot.Log.Info($"SpendingModule: Wood shortage fail cost {command.WoodCost}");
                        }
                        else if (command.FoodCost > 0 && food_shortage)
                        {
                            spend = false;
                            Bot.Log.Info($"SpendingModule: Food shortage fail cost {command.FoodCost}");
                        }
                        else if (command.GoldCost > 0 && gold_shortage)
                        {
                            spend = false;
                            Bot.Log.Info($"SpendingModule: Gold shortage fail cost {command.GoldCost}");
                        }
                        else if (command.StoneCost > 0 && stone_shortage)
                        {
                            spend = false;
                            Bot.Log.Info($"SpendingModule: Stone shortage fail cost {command.StoneCost}");
                        }
                    }

                    if (spend)
                    {
                        if (command.HasMessages)
                        {
                            Bot.Log.Info($"SpendingModule: Spending {command.WoodCost}/{command.FoodCost}/{command.GoldCost}/{command.StoneCost} (w/f/g/s) on: {command.Name}");
                            yield return command;
                        }
                    }
                    else
                    {
                        wood += command.WoodCost;
                        food += command.FoodCost;
                        gold += command.GoldCost;
                        stone += command.StoneCost;
                    }
                }
            }

            Commands.Clear();
        }

        protected override void Update()
        {
            
        }
    }
}
